...
"""sbin_gmmu_omo: plot per-benchmark normalized execution time vs baseline.

Reads the metrics CSV produced by 5_collect_metrics.py, normalizes each
config's kernel_time against the benchmark's baseline config
(normalized = t_config / t_baseline; 1.0 == baseline, > 1 slower, < 1 faster),
and writes a grouped bar chart (log y-scale because unified configs run up to
~150x slower than baseline) plus the normalized values as a CSV.
...
def main():
    ...
    for bench, base_t in base.items():
        for _, r in df[(df["benchmark"] == bench) &
            ...
            norm_rows.append({
                ...
                "normalized_time": r["kernel_time"] / base_t,
            })
    ...
    for off, cfg in zip(offsets, present_configs):
        sub = norm[norm["config"] == cfg].set_index("benchmark")["normalized_time"]
