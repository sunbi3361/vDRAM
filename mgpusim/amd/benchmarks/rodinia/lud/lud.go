...
// SimpleKernelArgs is the kernarg layout for lud_diagonal, lud_perimeter,
// and lud_internal on gcn3: the explicit args only. The OpenCL ABI kernel
// reads get_num_groups(0) from the dispatch packet (via the SGPR dispatch
// pointer), so the hidden global-offset fields of the 72-byte gcn3 kernarg
// buffer are left zeroed, which is correct for single-GPU.
...
func (b *Benchmark) initMem() {
	...
	b.driver.MemCopyH2D(b.context, b.gA, b.aOrig) // sbin_claude
}
