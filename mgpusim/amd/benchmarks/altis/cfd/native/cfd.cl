/*
 * OpenCL kernel for Altis CFD (gfx803 / GCN3)
 * Translated from altis_cfd.cpp for the clang-ocl -mcpu=gfx803 flow.
 *
...
static inline float compute_pressure(float rho, float mx, float my,
                                     float mz, float e) {
    float ke = 0.5f * (mx * mx + my * my + mz * mz) / fmax(rho, 1e-10f);
    return GAMMA_M1 * (e - ke);
}

static inline float compute_speed_of_sound(float rho, float p) {
    return sqrt(fmax(GAMMA * p / fmax(rho, 1e-10f), 1e-10f));
}

__kernel void compute_flux_kernel(
    __global const float* __restrict__ rho,
    __global const float* __restrict__ mx,
    __global const float* __restrict__ my,
    __global const float* __restrict__ mz,
    __global const float* __restrict__ energy,
    __global const int*   __restrict__ neighbors,  // [N * NUM_NEIGHBORS]
    __global const float* __restrict__ normals,    // [N * NUM_NEIGHBORS * 3]
    __global const float* __restrict__ areas,      // [N * NUM_NEIGHBORS]
    __global float* __restrict__ flux_rho,
    __global float* __restrict__ flux_mx,
    __global float* __restrict__ flux_my,
    __global float* __restrict__ flux_mz,
    __global float* __restrict__ flux_energy,
    int N)
{
    ...
    float inv_rho_i = 1.0f / fmax(rho_i, 1e-10f);
    ...
    for (int f = 0; f < NUM_NEIGHBORS; ++f) {
        ...
        float inv_rho_j = 1.0f / fmax(rho_j, 1e-10f);
        ...
        /* |vn| computed as sqrt(vn*vn): fabs lowers to an unimplemented
         * VOP3a opcode in the GCN3 emulator, and a branch-based abs was
         * inexact. sqrt is v_sqrt_f32 (implemented). */
        float avn_i = sqrt(vn_i * vn_i);
        float avn_j = sqrt(vn_j * vn_j);
        float lambda = fmax(avn_i + a_i, avn_j + a_j);
