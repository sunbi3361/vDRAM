...
func (b *Benchmark) loadProgram() {
	...
	b.hsaco = insts.LoadKernelCodeObjectFromBytes(hsacoBytes, "ep_kernel")
	if b.hsaco == nil {
...
func (b *Benchmark) initMem() {
	...
	if b.useManagedMemory { // sbin_claude
		b.gBinOut = b.driver.AllocateManaged(
...
// ln2f mirrors LN2_F in the gcn3 kernel (native_log is a base-2 log, so
// ln(u1) = log2(u1)*ln2 on the GPU).
const ln2f float32 = 0.69314718055994530942

// cpuBin reproduces, in float32 arithmetic, the exact bin index that the GPU
// kernel computes for a given work-item index. It mirrors the gcn3 kernel's
// instruction sequence: log2 (native_log) * ln2, sqrt, t = r*r, sqrt, trunc.
// (The gfx803 emulator lacks sin/cos, so the Box-Muller pair is folded into
// t = r*r = x1^2+x2^2, which is exact for any theta.)
func cpuBin(idx int) int32 {
	seed := uint32(idx + 1)
	seed = lcgNext(seed)
	seed = lcgNext(seed)
	seed = lcgNext(seed)
	// Mirrors the GPU's cvt_f32_u32 + mul by 2^-32 (single float32 rounding).
	u1 := float32(seed) * (1.0 / 4294967296.0)

	if u1 < 1e-10 {
		u1 = 1e-10
	}

	l2 := float32(math.Log2(float64(u1)))
	ln := l2 * ln2f
	r := float32(math.Sqrt(float64(-2.0 * ln)))
