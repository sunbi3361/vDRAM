...
// sbin_codex: region-scoped cache maintenance for the UVM control path. A
// write-through cache never holds dirty data, so a range writeback+invalidate
// degenerates to draining the matching in-flight requests and invalidating the
// matching lines. The cache is never paused.
